...
class ExperienceBufferWithImage:
    '''
    More generalized than replay buffers.
    Implemented for deep on-policy reinforcement learning algos.
    Store both image and state information in obses:
    env obses: {
        'image': (N, 1, Width, Hight), // for depth image
        'state': (N, num_obs)
    }
    '''
    def __init__(self, env_info, algo_info, device, aux_tensor_dict=None):
    ...
    def _init_from_env_info(self, env_info):
        ...
        self.tensor_dict['obses'] = {
            'image': torch.zeros(obs_base_shape + (1,  env_info['cam_resolution'][0], env_info['cam_resolution'][1]), dtype=torch.float32, device=self.device),
            'state': self._create_tensor_from_space(env_info['observation_space'], obs_base_shape)
        }
